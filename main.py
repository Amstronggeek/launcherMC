...
def main():
    ...
    def launch():
        ...
        # lis la version du launcher actuel
        with open("version", "r") as version_launcher:
            actual_version = version_launcher.read()
            version_launcher.close()

        url = "https://github.com/toniocraft/launcherMC/archive/refs/heads/version.zip"
        temp_file_path = os.path.join(tempfile.gettempdir(), "version_launcher" +
                                      str(random.randrange(1, 100000)) + ".zip")
        print(f"File downloaded : {temp_file_path}")
        urllib.request.urlretrieve(url, temp_file_path)

        # extract zip
        print("Extraction...")
        with ZipFile(temp_file_path, 'r') as ziptemp:
            ziptemp.printdir()
            path_extract = tempfile.gettempdir()
            ziptemp.extractall(path_extract)

        after_extract = os.path.join(tempfile.gettempdir(), 'launcherMC-version', 'version_launcher')
        with open(after_extract, "r") as \
                version_online:
            online_version = version_online.read()
            print(f"Version launcher : {actual_version}")
            print(f"Version online : {online_version}")
            if online_version != actual_version:
                print("Downloading the new version...")
                url_launcher = "https://github.com/toniocraft/launcherMC/releases/download/1.0/KipikCube.Installer.exe"
                urllib.request.urlretrieve(url_launcher, temp_file_path)
                subprocess.call(temp_file_path)

            version_online.close()
            os.remove(temp_file_path)
            shutil.rmtree(os.path.join(tempfile.gettempdir(), 'launcherMC-version'))

    # config setting window
    second_title = "Settings"
    second_geometry = '720x480'
    second_bg = '#2c2c2c'
